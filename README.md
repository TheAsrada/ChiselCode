...
| `/skills` | доступные скиллы: выбрать и посмотреть |
...
Где класть (первый найденный побеждает): `.chisel/skills/<имя>/` проекта → `.agents/skills/<имя>/` (общий стандарт, видят и другие агенты) → `skills/` рядом с конфигом → встроенные из установщика (`/review`, `/commit`, `/skill-creator`). Встроенные команды (`/settings` и др.) перекрыть нельзя. В журнале виден короткий `/review args`, выполняются инструкции из `SKILL.md`. Команда `/skills` показывает все доступные, подсказки и `/help` подхватывают новые скиллы сразу. Свой скилл-писатель — скилл `/skill-creator`.
