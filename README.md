...
1. На странице [Releases](https://github.com/TheAsrada/ChiselCode/releases) скачайте `ChiselCode-Setup-<версия>.exe` и запустите. Права администратора не нужны: установщик положит `chisel` в `%LOCALAPPDATA%\ChiselCode`, добавит его в PATH и создаст ярлык в меню «Пуск».
...
> Без установщика (portable-вариант): скачайте `chisel-windows-x64.exe`, положите в постоянную папку и вызывайте по полному пути вместо `chisel`.
...
- **Окно EXE сразу закрывается (мигает и исчезает)** — поставьте `ChiselCode-Setup-*.exe` из релизов и запускайте командой `chisel` из PowerShell. Portable-EXE тоже лучше запускать из консоли, а не двойным кликом. Начиная с 0.1.8 окно при двойном клике остаётся открытым с подсказкой и ждёт Enter, а `chisel "задача"` работает из кавычек.
