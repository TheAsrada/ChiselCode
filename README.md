...
Своя команда — это markdown-файл: имя файла даёт имя (`review.md` → `/review`), шаблон — текст запроса, `$ARGUMENTS` заменяется аргументами после команды. Необязательная шапка задаёт описание:

```markdown
---
description: Ревью незакоммиченных изменений
---
...
Где класть (первый найденный побеждает): `.chisel/commands/` проекта → `commands/` рядом с конфигом → встроенные из установщика (`/review`, `/commit`). Встроенные команды (`/settings` и др.) перекрыть нельзя. В журнале виден короткий `/review args`, выполняется развёрнутый шаблон. Подсказки и `/help` подхватывают новые файлы сразу.
