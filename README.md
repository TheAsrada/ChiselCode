...
- в старом conhost без признаков современного терминала — классика: история в scrollback, листает сам терминал;
- `CHISEL_ALT_SCREEN=0` или `CHISEL_NO_ALT_SCREEN=1` включает классику, `CHISEL_FORCE_ALT=1` принудительно включает полноэкранный режим; `CHISEL_NO_MOUSE=1` или `CHISEL_DISABLE_MOUSE=1` отключает захват мыши;
